...
`ml_trading_system.py:36` hard-codes four successful/attention-heavy tickers:
...
- `experiments/train_drl_v1.py:11-12` targets 80% high-confidence accuracy on four tickers.
- `experiments/train_pltr_ultra.py:23-26` is PLTR-only and explicitly targets 80% directional accuracy.
- `experiments/train_v7_deep.py:91-93` keeps `["NVDA", "AAPL", "PLTR", "TSLA"]`.
...
`ml_trading_system.py:44-85` embeds static fundamentals, analyst targets, and sentiment values. `ml_trading_system.py:186-194` adds them as constants to every historical row:
...
`ml_trading_system.py:249-254` says time-series CV/no lookahead, but fits the scaler on the full dataset before the train/test split:
...
- `experiments/self_improve.py:199-203` fits `StandardScaler` on all rows before the 80/20 split.
- `experiments/train_v3.py:468-489` fits the LSTM scaler on the full feature matrix before sequence splits.
- `experiments/train_v3.py:550-563` refits a full-data scaler for the latest forecast and saves it.
- `experiments/train_v3.py:585-587` scales the full classifier dataset.
- `experiments/train_v3.py:648-650` scales the full LightGBM regression dataset.
- `experiments/train_v3.py:688-692` scales the full RL environment feature matrix.
- `experiments/train_v4.py:675-681` scales full LSTM inputs before sequence split.
- `experiments/train_v4.py:809-814` scales full XGB inputs and computes class quantiles on full forward returns.
- `experiments/train_v4.py:868-930` scales full LGB/RF inputs before validation.
...
- Prediction and signal: `ml_trading_system.py:622-631`
- Immediate fill at same close: `ml_trading_system.py:632-649`
- Next-row valuation: `ml_trading_system.py:651-652`

The RL environment uses the current close in the observation and then executes at that same close:

- Observation price: `ml_trading_system.py:369-377`
- Execution price: `ml_trading_system.py:383-401`

Impact: if the model uses same-day candle features like body/wick/direction (`ml_trading_system.py:180-184`) and trades at the same close, it gets information that was not available at the fill.
...
`experiments/train_v4.py:958-984` builds meta-model features from base-model predictions over all rows, then fits the meta-model on the first 80%. The base models and preprocessing have already seen validation context:
...
`experiments/train_v8_finetune.py:1032-1043` describes a loop that compares to ground truth, adjusts weights, retrains, and re-predicts. `experiments/train_v8_finetune.py:1156-1239` then reruns predictions on the same result frame after failure analysis.

`experiments/train_v9_xgb.py:843-888` iterates up to 15 times and selects the best metrics; `experiments/train_v9_xgb.py:963-965` runs a five-iteration loop in main.
...
`experiments/train_drl_v1.py:545-615` computes forward labels and records only emitted BUY/SELL predictions. HOLDs are skipped:
...
`experiments/train_drl_v1.py:688-718` tries several DRL schedules and horizons, selecting the best high-confidence accuracy. `experiments/train_pltr_ultra.py:818-831` reports confidence-tier accuracy and Sharpe from selected signals; `experiments/train_pltr_ultra.py:920-954` tries three large PPO schedules and keeps the best.
...
- DRL v1 tries multiple PPO timestep/horizon configs (`experiments/train_drl_v1.py:688-718`).
- PLTR Ultra tries 150k/200k/250k schedules (`experiments/train_pltr_ultra.py:920-954`).
- v9 iterates and keeps the best run (`experiments/train_v9_xgb.py:843-888`).
...
`experiments/train_v7_deep.py:1-52` cites current academic/literature claims and vendor-style data. The market data loader says it uses realistic synthetic OHLCV (`experiments/train_v7_deep.py:240-245`) and generates correlated returns (`experiments/train_v7_deep.py:274-312`).

It also computes alpha IC using next-day returns inside the transformer approximation (`experiments/train_v7_deep.py:502-520`) and later evaluates against generated forward returns (`experiments/train_v7_deep.py:1458-1468`).
...
The old ML backtest includes a fixed 0.1% cost (`ml_trading_system.py:618`) but no spread, slippage, next-bar fill, or borrow/short constraints. Several old scripts report accuracy or selected-signal Sharpe rather than net portfolio returns.
